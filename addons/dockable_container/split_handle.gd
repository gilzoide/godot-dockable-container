...
func _draw() -> void:
	var theme_class := SPLIT_THEME_CLASS[DockableLayoutSplit.get_direction(layout_split)]
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if event.double_click:
			DockableLayoutSplit.set_percent(layout_split, 0.5)
	elif _dragging and event is InputEventMouseMotion:
		var mouse_in_parent := get_parent_control().get_local_mouse_position()
		if DockableLayoutSplit.is_horizontal(layout_split):
			DockableLayoutSplit.set_percent(layout_split, (
...
func _notification(what: int) -> void:
	if what == NOTIFICATION_MOUSE_ENTER:
		...
		if bool(get_theme_constant("autohide", SPLIT_THEME_CLASS[DockableLayoutSplit.get_direction(layout_split)])):
			queue_redraw()
...
func get_layout_minimum_size() -> Vector2:
	if not DockableLayoutSplit.is_split(layout_split):
		return Vector2.ZERO
	
	var separation := get_theme_constant("separation", SPLIT_THEME_CLASS[DockableLayoutSplit.get_direction(layout_split)])
	if DockableLayoutSplit.is_horizontal(layout_split):
		return Vector2(
...
func set_split_cursor(value: bool) -> void:
	if value:
		mouse_default_cursor_shape = SPLIT_MOUSE_CURSOR_SHAPE[DockableLayoutSplit.get_direction(layout_split)]
	else:
...
func get_split_rects(rect: Rect2) -> Dictionary:
	...
	var percent := DockableLayoutSplit.get_percent(layout_split)
	if DockableLayoutSplit.is_horizontal(layout_split):
